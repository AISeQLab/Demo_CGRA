...
#define MODE_BASE_IP	 (0x0000000000000020LL >> 2)
...
#define ROW0_MSB_BASE_PHYS	 (0x00000000 + PADDING_BASE)  
#define ROW0_LSB_BASE_PHYS	 (0x00008000 + PADDING_BASE) 
#define ROW1_MSB_BASE_PHYS	 (0x00010000 + PADDING_BASE)
#define ROW1_LSB_BASE_PHYS	 (0x00018000 + PADDING_BASE)
#define ROW2_MSB_BASE_PHYS	 (0x00020000 + PADDING_BASE)
#define ROW2_LSB_BASE_PHYS	 (0x00028000 + PADDING_BASE)
#define ROW3_MSB_BASE_PHYS	 (0x00030000 + PADDING_BASE)
#define ROW3_LSB_BASE_PHYS	 (0x00038000 + PADDING_BASE)
...
//** For Transfer on ARM ZYNQ **//
  ...
  U64  *ctx_rc_mmap;     // user-virt Contiguous 4GB space including LMM space
  U64  *ctx_im_mmap;     // user-virt Contiguous 4GB space including LMM space
  U64  ctx_pe_offset;
  U64  ctx_rc_offset;
  U64  ctx_im_offset;  
...
int cgra_open()
{
  ...
  for (dir = 0; dir < num_dirs; ++dir) {
	...
	return -1;
	  ...
	  CGRA_info.ctx_rc_offset = CTX_RC_BASE_IP >> 3;
      CGRA_info.ctx_rc_mmap = (U64*)mmap(NULL, REG_MMAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd_reg, 0); /* 4GB */
      if (CGRA_info.ctx_rc_mmap == MAP_FAILED) {
		printf("ctx_rc_mmap failed. errno=%d\n", errno);
		return -1;
      }

	  CGRA_info.ctx_im_offset = CTX_IM_BASE_IP >> 3;
      CGRA_info.ctx_im_mmap = (U64*)mmap(NULL, REG_MMAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd_reg, 0); /* 4GB */
